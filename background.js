  ...
  co(function *(){
    ...
    for (var k in window.Storage.bit) {
      if(localStorage[k] && JSON.parse(localStorage[k])){
        ...
        log(k)
        log(draw(parseFloat(v.last), sta(parseFloat(v.last), data.bids, data.asks).map(function(e){ return [e.index, e.vol] })).join("-->"))
...
function showDrawLine(data){
  if(data.length < 1){
    return data
  }
  var pre = data[0],
      result = []
  result.push(pre.toFixed(5))
